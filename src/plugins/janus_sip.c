...
#define AES_KEY_LENGTH 32  /* AES-256 = 32 octets */
#define BUFFER_SIZE 1024   /* Taille maximale pour le texte chiffré */
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "register")) {
			...
			if(secret) {
				...
				if(secret_encryption_key) {
					const char *secret_text = decrypt_aes_secret(secret_encryption_key, json_string_value(secret));
					if(secret_text == NULL) {
